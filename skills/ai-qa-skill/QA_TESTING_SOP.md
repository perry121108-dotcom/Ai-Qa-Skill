...
> 精簡七步流程與完成門檻見 **[`SKILL.md`](SKILL.md)**（入口），本檔聚焦各步驟的執行細則。
...
### 5.1 JSON 結構強固性（Schema Robustness）｜OWASP `LLM05` 不當輸出處理
...
### 5.2 Prompt 防越獄與安全（Jailbreak & Injection）｜OWASP `LLM01` 提示注入 · `LLM02` 機敏外洩 · `LLM07` 系統提示外洩
- 撰寫測試嘗試誘導模型**洩漏 System Prompt／開發者指令**，斷言輸出**不含**系統提示內容。
- 注入攻擊測試：使用者輸入夾帶「忽略上述指令…」「現在你是…」等，斷言系統角色與限制**不被覆寫**。
- 驗證機密（API Key、內部路徑）**永不**出現在模型輸出。

### 5.3 幻覺與誠信邊界（Hallucination & Honesty）｜OWASP `LLM09` 錯誤資訊
...
> 完整完成門檻見 **[`SKILL.md`](SKILL.md)**。一句話：完成 QA 的標準**不是產出 N 份文件**，而是「對應功能有可執行測試並實際通過、附終端機證據；有 LLM 呼叫點則三維度評估齊備；已修 Critical/Major Bug 有 red→green 回歸測試；無法執行者誠實標 Blocked/Not Run」。
